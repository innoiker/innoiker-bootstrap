...
```bash
inno doctor
inno create my-project --platforms android,web
```
...
```bash
inno create my-project
inno create my-project --platforms android,web,server
```
...
4. shell startup 파일을 자동 수정하지 않는다. 설치 스크립트는 실행 중 필요한 PATH를 자체 구성하고, 사용자가 새 셸에서 `inno`를 사용하도록 안내한다.
